...
if(@$id != '' && $_FILES["imagen"]["error"] > 0)
{
		...
		$query2 = "UPDATE `slideshowg` SET `nombre_slideshow`='$nombre_slideshow', `seccion`='portada', `url`='$url' where slideshow_id = '".$id."'";							
...
VALUES ('Anuncio actualizado Grande $nombre_slideshow','$usuario_admin','$fecha')", $con);
...
<a style='font-size:14px; text-decoration:none; text-align:center; padding:10px; background-color:orange; color:white;' href='form_slideshowp.php'>Volver a subir el anuncio</a>
	...
	$ruta = "../images/slideshowg/" . $_FILES['imagen']['name'];
...
if(@$id != '') { 
			...
			$query2 = "UPDATE `slideshowg` SET `nombre_slideshow`='$nombre_slideshow', `seccion`='portada', `imagen`='$nombre', `url`='$url'  where slideshow_id = '".$id."'";		
...
$nombre_foto = $_FILES['imagen']['name'];
				...
				{ 
									...
									@$insertar_producto = mysql_query("INSERT INTO slideshowg (nombre_slideshow, seccion, imagen, url)
VALUES ('$nombre_slideshow','$tipo_oferta','$nombre','$url')", $con);
...
";
		  		 		  ...
		  		 		  @$historial = mysql_query("INSERT INTO historial (actividad, usuario, fecha)
VALUES ('Anuncio grande $nombre_slideshow','$usuario_admin','$fecha')", $con);
...
VALUES ('Anuncio actualizado Grande $nombre_slideshow','$usuario_admin','$fecha')", $con);
		  ...
		  echo "<h3 style='text-align:center; color:red; margin-top:100px;'>El anuncio ha sido actualizada exitosamente<br><br>
<a style='font-size:14px; text-decoration:none; text-align:center; padding:10px; background-color:orange; color:white;' href='slideshowg.php'>Ir a todos los anuncios</a></h3>
		  ";
		  }
